...
- **Key:** `AGP`
- **Board:** https://procyoncreative.atlassian.net/jira/software/c/projects/AGP/boards
...
- **Email:** your Atlassian email (set as the `JIRA_EMAIL` repo secret and used by MCP)
...
- **No work without a ticket.** Every branch must reference an `AGP-NNN` Jira ticket. If one doesn't exist, create it first via `/jira-ticket create` (or in the Jira UI) before opening the branch.
- **Branch name format:** `AGP-NNN-short-description` (e.g. `AGP-42-add-login-form`).
- **Ticket requirements:**
