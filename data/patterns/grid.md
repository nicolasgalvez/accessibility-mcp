...
id: "grid"
patternDocUrl: "https://www.w3.org/WAI/ARIA/apg/patterns/grid/"
---
...
*   A `grid` is a composite widget so it:
    ...
    *   Requires the author to provide code that [manages focus movement inside it](../../practices/keyboard-interface/keyboard-interface-practice.html#kbd_general_within).
...
Because arrow keys are used to move focus inside of a `grid`, a `grid` is both easier to build and use if the components it contains do not require the arrow keys to operate. If a cell contains an element like a [listbox](../listbox/listbox-pattern.html), then an extra key command to focus and activate the listbox is needed as well as a command for restoring the grid navigation functionality. Approaches to supporting this need are described in the section on [Editing and Navigating Inside a Cell](#gridNav_inside).
...
*   Enter: Disables grid navigation and:
    *   If the cell contains editable content, places focus in an input field, such as a [textbox](#textbox). If the input is a single-line text field, a subsequent press of Enter may either restore grid navigation functions or move focus to an input field in a neighboring cell.
    *   If the cell contains one or more widgets, places focus on the first widget.
*   F2:
    *   If the cell contains editable content, places focus in an input field, such as a [textbox](#textbox). A subsequent press of F2 restores grid navigation functions.
...
*   If there is an element in the user interface that serves as a label for the grid, [aria-labelledby](#aria-labelledby) is set on the grid element with a value that refers to the labelling element. Otherwise, a label is specified for the grid element using [aria-label](#aria-label).
...
*   If the grid provides sort functions, [aria-sort](#aria-sort) is set to an appropriate value on the header cell element for the sorted column or row as described in the [Grid and Table Properties Practice](../../practices/grid-and-table-properties/grid-and-table-properties-practice.html#gridAndTableProperties_sort).
*   If the grid supports selection, when a cell or row is selected, the selected element has [aria-selected](#aria-selected) set `true`. If the grid supports column selection and a column is selected, all cells in the column have `aria-selected` set to `true`.
