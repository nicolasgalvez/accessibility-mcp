...
id: "spinbutton"
patternDocUrl: "https://www.w3.org/WAI/ARIA/apg/patterns/spinbutton/"
---
...
*   The focusable element serving as the spinbutton has role [spinbutton](#spinbutton). This is typically an element that supports text input.
...
*   If the spinbutton has a visible label, it is referenced by [aria-labelledby](#aria-labelledby) on the spinbutton element. Otherwise, the spinbutton element has a label provided by [aria-label](#aria-label).
*   The spinbutton element has [aria-invalid](#aria-invalid) set to `true` if the value is outside the allowed range. Note that most implementations prevent input of invalid values, but in some scenarios, blocking all invalid input may not be practical.
